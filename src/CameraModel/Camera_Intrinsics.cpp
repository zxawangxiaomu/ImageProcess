...
/// 设置相机参数
bool CameraIntrinsics::setCameraParams(const std::vector<double>& params)
{
    if (params.size() != 4)
    {
        return false;
...
// 加畸变
Vec2 CameraIntrinsics::addDistortion(const Vec2& ud_cam_pt)
{
    return ud_cam_pt;
}
// 去畸变
Vec2 CameraIntrinsics::removeDistortion(const Vec2& cam_pt)
{
    return cam_pt;
}
